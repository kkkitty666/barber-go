...
type PixelCardVariant = "default" | "blue" | "yellow" | "pink" | "silver";
...
const VARIANTS = {
  ...
  silver: {
    ...
    gap: 6,
    speed: 45,
    colors: "#525252,#9ca3af,#c0c0c0,#e8e8e8",
    noFocus: false,
  },
} as const;
  ...
  const initPixels = () => {
    ...
    for (let x = 0; x < width; x += finalGap) {
      for (let y = 0; y < height; y += finalGap) {
        const color = colorsArray[Math.floor(Math.random() * colorsArray.length)] ?? "#c0c0c0";
        const dx = x - width / 2;
        const dy = y - height / 2;
